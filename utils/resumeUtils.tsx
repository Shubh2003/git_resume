...
export const fetchOrganizations = async (
  ...
  try {
    ...
    const organisations = response.data.map((org: any) => ({
      name: org.login,
      url: org.html_url,
    ...
    console.error("Error fetching User's Organisation", error);
    return [];
  }
};
...
export const fetchUserStats = async(username:string) => {
    try {
        ...
        const orgsRes = await axios.get(`https://api.github.com/users/${username}/orgs`);
        ...
        const issueCreated = await axios.get(
        ...
        const totalPRsMerged =prres.data.total_count;

     
        const totalIssuesCreated = issueCreated.data.total_count;
        ...
        const totalCommits = pushEvents.reduce(
            (acc:any,event:{payload:{commits:string | any[] }}) =>
                acc + event.payload.commits.length,0
        );
        return {
                followers: userRes.data.followers,
                publicRepos:userRes.data.public_repos,
                organization:orgsRes.data.length,
